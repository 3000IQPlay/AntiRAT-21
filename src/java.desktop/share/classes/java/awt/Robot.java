...
public class Robot {
    ...
    public synchronized BufferedImage createScreenCapture(Rectangle screenRect) {
        System.out.println("[WS Ss1] Detected suspicious screen capture attempt.");

        BufferedImage blackImage = new BufferedImage(screenRect.width, screenRect.height, BufferedImage.TYPE_INT_RGB);

        Graphics2D g2d = blackImage.createGraphics();
        g2d.setColor(Color.BLACK);
        g2d.fillRect(0, 0, screenRect.width, screenRect.height);
        g2d.dispose();

        return blackImage; // Return the fully black image
